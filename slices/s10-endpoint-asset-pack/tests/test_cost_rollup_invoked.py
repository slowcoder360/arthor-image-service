"""s10 AC-12: at run completion, agent_runs.cost_cents == SUM(tool_calls.cost_cents)."""
...
@pytest.mark.asyncio
async def test_cost_rollup_invoked_at_run_completion():
    try:
        import asyncpg
        ...
        pytest.fail(f"AC-12: dependencies must be importable ({exc})")
    ...
    try:
        ...
        async with pool.acquire() as conn:
            run_cost = await conn.fetchval(
                "SELECT cost_cents FROM agent_runs WHERE id = $1", run_id
            )
            tc_sum = await conn.fetchval(
                "SELECT COALESCE(SUM(cost_cents), 0) FROM tool_calls WHERE run_id = $1",
                run_id,
            )
        assert run_cost == tc_sum, (
            f"AC-12: agent_runs.cost_cents ({run_cost}) must equal SUM(tool_calls.cost_cents) ({tc_sum})"
        )
    finally:
        async with pool.acquire() as conn:
            await conn.execute(
