"""s14 AC-8: the inspector's internal call to /images/regenerate-slot carries
a valid X-Arthor-Signature (round-trip verified via verify_signature).
"""

from __future__ import annotations

import os
import uuid

import pytest

from _s14_db_helpers import cleanup_run, make_pool, seed_uploaded_asset


@pytest.mark.requires_db
@pytest.mark.asyncio
async def test_inspector_outbound_call_is_signed(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("FASTAPI_ARTHOR_SHARED_SECRET", "shared-key-123")
    monkeypatch.setenv("INSPECTOR_ADMIN_TOKEN", "secret-admin-token")
    monkeypatch.setenv("DATABASE_URL", os.environ["DATABASE_URL"])
    try:
        from app.auth.hmac import verify_signature  # type: ignore[import-not-found]
        from app.inspector import router as inspector_router_mod  # type: ignore[import-not-found]
        from app.main import app  # type: ignore[import-not-found]
    except ImportError as exc:
        pytest.fail(f"AC-8: app + inspector.router + verify_signature must be importable ({exc})")
    try:
        from httpx import ASGITransport, AsyncClient
    except ImportError as exc:
        pytest.fail(f"httpx not installed: {exc}")

    pool = await make_pool()
    run_id = None
    asset_id = None
    captured = {}
    try:
        async with pool.acquire() as conn:
            run_id, asset_id = await seed_uploaded_asset(conn)

        class FakeClient:
            async def post(self, url, content=None, headers=None, **kw):
                captured["url"] = url
                captured["body"] = content
                captured["headers"] = headers or {}

                class _R:
                    status_code = 202

                    def json(self_):
                        return {
                            "agent_run_id": str(uuid.uuid4()),
                            "new_asset_id": str(uuid.uuid4()),
                            "status": "accepted",
                        }

                return _R()

            async def __aenter__(self):
                return self

            async def __aexit__(self, *exc):
                return False

        monkeypatch.setattr(
            inspector_router_mod,
            "inspector_http_client",
            lambda *a, **kw: FakeClient(),
            raising=False,
        )

        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://testserver") as client:
            get_login = await client.get("/inspector/login")
            csrf = get_login.cookies.get("arthor_csrf_token") or ""
            client.cookies.set("arthor_csrf_token", csrf)
            resp = await client.post(
                f"/inspector/slots/{asset_id}/regenerate",
                data={"prompt_modifier": "x", "csrf_token": csrf},
                headers={"Authorization": "Bearer secret-admin-token"},
            )
        assert resp.status_code == 200, (
            f"AC-8: inspector POST must 200; got {resp.status_code}"
        )
        headers = {k.lower(): v for k, v in (captured.get("headers") or {}).items()}
        sig = headers.get("x-arthor-signature")
        assert sig, (
            f"AC-8: outbound regenerate call must carry X-Arthor-Signature header; "
            f"got headers={captured.get('headers')!r}"
        )
        body = captured.get("body") or b""
        if isinstance(body, str):
            body = body.encode()
        verify_signature("shared-key-123", body, sig)
    finally:
        async with pool.acquire() as conn:
            if run_id is not None:
                await cleanup_run(conn, run_id)
        await pool.close()
