...
def _variant_entry(
    ...
    *,
    ...
    industry = resolve_industry(row.get("industry") or row.get("slug") or "general").label
    scene = INDUSTRY_VISUAL_TRIAD.get(industry, INDUSTRY_VISUAL_TRIAD["general_services"])[idx]
    ...
    r2_key = row.get("r2_key") or f"corpus/{industry}/{idx}.webp"
    return {
        "variant_index": idx,
        ...
        "compiler_version": row.get("compiler_version") or "3.4",
...
def write_corpus_yaml(
    ...
    *,
    ...
    path = out_dir / f"{industry_label}.yaml"
